function [theta_b, dMSE_b] = BAd_GVAMP_c_known(Ai, A0, y, T_LMMSE, T_VN_denoising, x_true,b_true, NumBits,delta)
% Code for paper:"Bilinear Adaptive Generalized Adaptive Vector Approximate
% Message Passing", IEEE Access, 2018. 
% main function for BAd-GVAMP algorithm (L = 1 case) with c known
...
%         cb = 1;
    ...
    for k0 = 1:T_LMMSE
        ...
        for qi = 1:Q 
            for qj = 1:Q
                H(qi,qj) = trace(Ai(:,:,qj)'*Ai(:,:,qi)*(Cov_inv+xhat2k*xhat2k'));
            end
            beta(qi) = trace(y_tilde'*Ai(:,:,qi)*xhat2k)-trace(A0'*Ai(:,:,qi)*(Cov_inv+xhat2k*xhat2k'));
        end
        theta_b = H\beta;
